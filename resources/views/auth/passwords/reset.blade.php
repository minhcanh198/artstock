...
<div class="container-fluid margin-bottom-40">
	...
	<div class="login-form">
		...
		@include('errors.errors-forms')
          	...
          	<form action="{{{ url('/password/resets') }}}" method="post" name="form" id="signup_form">
            
            <input type="hidden" name="_token" value="{{{ csrf_token() }}}">
            <input type="hidden" name="token" value="{{ $token }}">
            
            <div class="form-group has-feedback">
              <input type="text" class="form-control login-field custom-rounded" name="email" id="email" value="{{ $emailAddress }}" placeholder="{{{ trans('auth.email') }}}" title="{{{ trans('auth.email') }}}" autocomplete="off">
