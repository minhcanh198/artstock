...
@if( Auth::check() && Auth::user()->id == $user->id )
	<!-- *********** COVER ************* -->
      <form class="pull-left myicon-right position-relative" style="z-index: 100;" action="{{url('upload/cover')}}" method="POST" id="formCover" accept-charset="UTF-8" enctype="multipart/form-data">
			...
			<button type="button" class="btn btn-default btn-border btn-sm" id="cover_file" style="margin-top: 10px; background-color: #fff; padding: 4px 14px;">
...
<div class="col-md-12">
	...
	<div class="center-block text-center profile-user-over">
		 ...
		 <a href="{{ url($user->username) }}">
        		<img loading="lazy" src="{{ asset('avatar').'/'.$user->avatar }}" width="150" height="150" class="img-circle border-avatar-profile avatarUser" />
        		</a>
		 </div>
        ...
        <a href="{{ $user->personal_website }}">
		  ...
		  <small style="display:block;"><a href="{{ $user->personal_website }}">{{  $user->personal_website }}</a></small>
		  @endif
...
@if( Auth::check() && Auth::user()->id == $user->id )
	<!-- *********** AVATAR ************* -->
      <form action="{{url('upload/avatar')}}" method="POST" id="formAvatar" accept-charset="UTF-8" enctype="multipart/form-data">
    		<input type="hidden" name="_token" value="{{ csrf_token() }}">
			<button type="button" class="btn btn-default btn-border btn-sm" id="avatar_file" style="margin-top: 10px; background-color: #fff; padding: 4px 14px;">
	    		<i class="icon-camera fa fa-camera myicon-right" ></i> {{ trans('misc.change_avatar') }}
	    		</button>
	    		 <input type="file" name="photo" id="uploadAvatar" accept="image/*" style="visibility: hidden; display: block;">
			</form><!-- *********** AVATAR ************* -->
			<form id="personalWebForm" action="{{ url('add/personal') }}" method="post" enctype="multipart/form-data" style="margin-bottom:2em;">
			    @csrf
              <div class="changePass">
                <button type="button" class="changePassBtn btn btn-success">{{ (Auth::user()->personal_website)?Auth::user()->personal_website:'Add Personal Website' }} <i class="fa fa-pencil"></i></button>
		...
		<h4 class="text-bold line-sm position-relative font-family-primary font-default text-muted mb-4" style="font-size: 22px;">{{ trans('users.about_me') }}</h4>
           ...
           @if( $user->bio != '' )
           <h4 class="text-center bio-user none-overflow" style="font-size: 18px;">{{ e($user->bio) }}</h4>
           @endif
