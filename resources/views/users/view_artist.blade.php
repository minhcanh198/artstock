                ...
                <div class="row mb-5">
                    <div class="col-md-3">
                        <div class="profile-inner">
                            <div class="img">
                                <img loading="lazy" src="<?php echo url('/') . '/avatar/' . $artistDetails->avatar; ?>"
                                     class="img-fluid">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="text2 text-person-photographer">
                            ...
                            @if(!empty($artistDetails->personal_website))
                                <small style="display:block;"><a
                                        href="{{ $artistDetails->personal_website }}">{{  $artistDetails->personal_website }}</a></small>
                            @endif
                            <p class="one">
                            ...
                            <input type="hidden" id="latlngValue" name="latlngValue"
                                   value="<?php echo $artistDetails->lat . ',' . $artistDetails->lng;?>">
                            <div class="d-flex mt-3">
                                <start-chat :receiver-id="{{ $artistDetails->id }}"></start-chat>
                            </div>
                            @if($artistDetails->langauges_speak != "" || $artistDetails->fvt_place_to_shoot != "")
                                <div class="profile-info-area mt-4">
                                    @if($artistDetails->langauges_speak != "")
                                        <p class="mb-3"><i class="fas fa-language mr-2"></i> <span class="language">Languages Spoken:</span> {{ $artistDetails->langauges_speak }}
                                        </p>
                                    @endif
                                    @if($artistDetails->fvt_place_to_shoot != "")
                                        <p><i class="far fa-heart mr-2"></i> <span class="language">Favourite Place to Shoot:</span> {{ $artistDetails->fvt_place_to_shoot }}
                                        </p>
                                    @endif
                                </div>
                            @endif
...
@endif
                ...
                <div class="customer-reviews-area">
                    ...
                    <ul class="row portfolio-list-one">
                        ...
                        <li class="col-md-3 mb-3 col-6 position-relative">
                            ...
                            <div class="img-overlay-price">
                                <a data-fancybox data-price="{{ $ImageData->price }}" data-id="{{ $ImageData->id }}"
                                   data-title="{{ $ImageData->title }}" data-slug="{{ str_slug($ImageData->title) }}"
                                   data-description="{{ $ImageData->description }}"
                                   data-typee="{{ $ImageData->is_type }}"
                                   href="{{ asset($thumbnail) }}">{{$settings->currency_symbol.$ImageData->price}}</a>
                            </div>
                        </li>
                        <?php
                        }
                    ...
                    <ul class="row portfolio-list-one">
                        ...
                        <li class="col-md-3 mb-3 col-6">
                            <a data-fancybox
                               href="{{ asset($watermarkedVideoPath) }}{{ '/screen-shot-'.$AnimationFileScreenShotName.'.png' }}">
                                <img loading="lazy"
                                     src="{{ asset($watermarkedVideoPath) }}{{ '/screen-shot-'.$AnimationFileScreenShotName.'.png' }}"
                                     class="img-fluid img">
                            </a>
                        </li>
                        <?php
                    ...
                    <ul class="row portfolio-list-one">
                        ...
                        <li class="col-md-3 mb-3 col-6">
                            <a data-fancybox
                               href="{{ asset($watermarkedVideoPath) }}{{ '/screen-shot-'.$VideoFileScreenShotName.'.png' }}">
                                <img loading="lazy"
                                     src="{{ asset($watermarkedVideoPath) }}{{ '/screen-shot-'.$VideoFileScreenShotName.'.png' }}"
                                     class="img-fluid img">
                            </a>
                        </li>
                        <?php
                ...
                <!-- Audio start -->
                    <div class="audio-song-box Bigwave"
                         data-path="<?php echo url('/uploads/audio/large/') . '/' . $AudioData->thumbnail; ?>">
                        ...
                        <div class="row music-main-page-home">
                            <div class="align-self-center col-md-2 text-center">
                                ...
                                <a href="javascript:;" class="btn-music-play" id="Bigbaton-pauseMusic#{{$AudKey}}"
                                   style="display: none;">
                        ...
                        @foreach($getReviewOfUserById as $reviewResult)
                            <div class="row customer-review-box">
                                <div class="col-3 pr-0">
                                    <img loading="lazy"
                                         src="<?php echo url('/') . '/review_images/' . $reviewResult->review_image; ?>"
                                         class="img-fluid  unique-viewartist-img">
                                </div>
                                <div class="col-9">
                                    <div class="d-flex mt-3">
                                        <div class="">
                                    ...
                                    <p class="unique-viewartist-para"
                                       style="word-break: break-all; font-size: 14px;">{{ $reviewResult->review_description }}</p>
                                </div>
                            </div>
                        @endforeach
                    @else
                ...
                <div class="border-calendar">
                    ...
                    <div class="calender-btn text-center mt-4">
                        <form id="formReqForm" method="get" action="<?php echo url('/') . '/request-to-book/' ?>">
                            ...
                            <input type="text" hidden id="photographerId" name="photographerId"
                                   value="{{ $artistDetails->id }}">
                            <button id="btnReq" disabled>Request to
                                book {{ ($artistDetails->name != "") ? $artistDetails->name : $artistDetails->username }}</button>
                        </form>
                    </div>
                    <hr>
                    <div class="celendar-inner">
                        ...
                        <p class="mb-3 disc-calender">
                            The availability of artists can vary from the dates you have requested. Within 24 hours of
                            receiving your request, the artist will confirm the date and time.
                        </p>
                        <p class="mb-3 disc-calender">
                            Artists are available for booking for a minimum 30 & 60 minutes session at your requested
                            place.
                        </p>
