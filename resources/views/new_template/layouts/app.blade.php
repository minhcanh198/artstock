            						...
            						<li>
            						    ...
            						    <ul class="mobile-nav-cate-ul">
            						        @foreach(  App\Models\Types::where('mode','on')->orderBy('type_name')->get() as $type )
												<li><a href="{{ url('category') }}/artist-{{ $type->type_name }}">{{ $type->type_name }}</a></li>
											@endforeach
            						        <!--<li><a href="#">One</a></li>-->
            						        <!--<li><a href="#">Two</a></li>-->
										...
										<ul class="dropdown-menu new-drop dd-close arrow-up nav-session" dir="RTL" role="menu" aria-labelledby="dropdownMenu4">
											<div class="row">
												...
												<div class="col-6">
													...
													@foreach(  App\Models\Types::where('mode','on')->orderBy('type_name')->get() as $type )
														<li class="pb-2"><a href="{{ url('category') }}/artist-{{ $type->type_name }}">{{ $type->type_name }}</a></li>
													@endforeach
												</div>
												<div class="col-6">
													...
													@foreach(  App\Models\Categories::where('mode','on')->where('link_with' , '!=', '0')->where('parent_id','=','0')->orderBy('name')->take(9)->get() as $category )
														@if($category->name != "Uncategorized")
															<li class="pb-2"><a href="{{ url('category') }}/{{ $category->slug }}">{{ $category->name }}</a></li>
														@endif
													@endforeach
												</div>
											</div>
        ...
        <!--                            </div>-->
								...
								<!--<li><a href="javascript:;" id="openModalMessageChat" class="chat-btn-home"><i class="fas fa-comments"></i><span id="spanMessageCounter" class="chat-icon-notification">0</span></a></li>--> <!-- Commented by shahzad-->
									...
									<li class="dropdown new">
										...
										<!-- DROPDOWN MENU -->
										<ul class="dropdown-menu new-drop dd-close arrow-up nav-session" role="menu" aria-labelledby="dropdownMenu4">
									...
									@if(\Request::segment(1) != "destinations")
										...
										@if( Auth::user()->authorized_to_upload == 'yes' )
											...
											<div class="dropdown-one-one">
												<button onclick="myFunction()" class="dropbtn-one-one" style="padding: 12px 20px 12px 20px;"><i class="fa fa-upload"></i> Upload</button>
								...
								<div class="social-icons">
									<ul>
										...
										@if($socialMedia[0]->instagram != "")
											<li><a href="{{ $socialMedia[0]->instagram }}"><i class="fab fa-instagram"></i></a></li>
										@endif
										@if($socialMedia[0]->facebook != "")
											<li><a href="{{ $socialMedia[0]->facebook }}"><i class="fab fa-facebook"></i></a></li>
										@endif
										@if($socialMedia[0]->pinterest != "")
											<li><a href="{{ $socialMedia[0]->pinterest }}"><i class="fab fa-pinterest"></i></a></li>
										@endif
										@if($socialMedia[0]->youtube != "")
											<li><a href="{{ $socialMedia[0]->youtube }}"><i class="fab fa-youtube"></i></a></li>
										@endif
										@if($socialMedia[0]->linkedin != "")
											<li><a href="{{ $socialMedia[0]->linkedin }}"><i class="fab fa-linkedin"></i></a></li>
										@endif
									</ul>
								</div>
							</div>
			...
			$(document).ready(function() {
                ...
                var handler = StripeCheckout.configure({
                  ...
                  token: function(token) {
                    ...
                    $('.instantBuyBtn').parents('form').append($('<input>').attr({ type: 'hidden', name: 'stripeToken', value: token.id })).submit();
                  },
                  opened: function() {
