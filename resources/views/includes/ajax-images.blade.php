...
@if($images->count() > 0)
    <section id="photos" class="baguetteBoxOne gallery ">
        @foreach($images as $image)
                    ...
                    <a href="{{ url('video', $image->id ) }}/{{str_slug($image->title)}}">
                        <video onmouseover="this.play()" onmouseout="this.pause()" width="320" height="240" muted loop>
                            @if($image->extension == "mp4")
                                <source src="{{ asset($watermarkedVideoPath) }}{{ '/watermark-'.$image->thumbnail }}"
                                        type="video/mp4">
                        @endif
            ...
            @else
                @if($i == 0)
                    <div class="box"><a href="{{ url('photo', $image->id ) }}/{{str_slug($image->title)}}"><img
                                loading="lazy" class="img-fluid" src="{{ asset($thumbnail) }}"></a></div>
                @endif

                @if($i==1 || $i == 2)
                    @if($i == 1)
                        <div class="box2"><a href="{{ url('photo', $image->id ) }}/{{str_slug($image->title)}}"><img
                                    loading="lazy" class="img-fluid" src="{{ asset($thumbnail) }}"></a></div>
                    @endif
                    @if($i == 2)
                        <div class="box2"><a href="{{ url('photo', $image->id ) }}/{{str_slug($image->title)}}"><img
