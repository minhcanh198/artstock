        	...
        	<div class="box box-danger">
                ...
                <form class="form-horizontal" method="post" action="{{{ url('panel/admin/categories/update') }}}" enctype="multipart/form-data">
                 ...
                 <!-- Start Box Body -->
                  ...
                  <div class="box-body">
                    <div class="form-group">
                      <label class="col-sm-2 control-label">{{{ trans('admin.icontext') }}}</label>
                      <div class="col-sm-10">
                        <input type="text" value="{{{ $categories->icon_text }}}" name="icon_text" class="form-control" placeholder="{{{ trans('admin.icontext') }}}">
                      </div>
                    </div>
                  </div><!-- /.box-body -->
                  ...
                  <div class="box-body">
                    <div class="form-group">
                      <label class="col-sm-2 control-label">{{{ trans('admin.link_with') }}}</label>
                      <div class="col-sm-10">
                        <select id="link_with" name="link_with" class="form-control">
                          <option value="">Select Type</option>
                          @foreach ($typeData as $types)
                            @if($types->types_id == $categories->link_with)
                              <option selected value="{{ $types->types_id }}">{{ $types->type_name }}</option>
                            @else
                              <option value="{{ $types->types_id }}">{{ $types->type_name }}</option>
                            @endif
                          @endforeach
                        </select>
                      </div>
